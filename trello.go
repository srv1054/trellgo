...
func dumpABoard(config Config, board *trello.Board, client *trello.Client) {
	...
	*/
	...
	if board.Prefs.BackgroundImage != "" {
		...
		localFilePath := filepath.Join(boardPath, "BoardBackground-")
	...
	*/
	...
	if err != nil {
		...
		labelFileName := filepath.Join(boardPath, "BoardLabels.md")
	...
	*/
	...
	if err != nil {
		...
		// Write buffer content to a file
		memberFileName := filepath.Join(boardPath, "BoardMembers.md")
	...
	*/
	...
	for x, card := range cards {
		...
		dirCreate(filepath.Join(boardPath, cleanListPath))
		...
		if card.Closed {
			if !config.ARGS.SeparateArchived {
				...
				cardPath = filepath.Join(boardPath, cleanListPath, cleanCardPath+" (ARCHIVED)")
				// If -split flag is set, move to ARCHIVED directory
			} else {
				cardPath = filepath.Join(boardPath, "ARCHIVED", cleanListPath, cleanCardPath)
			}
			// card is not archived
		} else {
			cardPath = filepath.Join(boardPath, cleanListPath, cleanCardPath)
		}
		...
		*/
		...
		if err != nil {
			logger("Error: Unable to get attachment data for card ID "+card.ID+": "+err.Error(), "err", true, false, config)
		}
